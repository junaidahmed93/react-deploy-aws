...
import DriporterTable from '../../components/driporter/DriporterTable';
import GlobalStyle from '../../utils/Styles';
import { nextBookings, previousBookings, startRecord, endRecord, totalRecords } from '../../utils/Pagination';

class DriporterContainer extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    this.props.actions.getDriporter();
  }
    ...
    return (
      <div id="vehicleContainer">
        ...
        <Paper style={GlobalStyle.containerPaperStyle} zDepth={0}>
          <Grid fluid className="container-no-padding">
            <Row between="xs">
              <Col xs={6} style={GlobalStyle.containerHeader}>
                <h2 className="paper-title ">Driporter Overview</h2>
              </Col>
              <Col xs={6}>
                <div style={{ float: 'right' }}>
                  <TextField
                    className="search-text-field"
                    hintText="Driporter Name"
                    floatingLabelText="Search"
                    onChange={this.onSearchChanged}
                  />
                  <Link to="/home/admin/driporter/add">
                    <FlatButton label="New Driporter" className="add-button-on-header float-right" />
                  </Link>
                </div>
          ...
          <DriporterTable rows={this.state.shownRecords} />
...
export default connect(mapStateToProps, mapDispatchToProps)(DriporterContainer);
