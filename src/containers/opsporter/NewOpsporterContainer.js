...
import OpsporterForm from '../../components/opsporter/opsporterForm';
import { formsValidation } from '../../utils/Helpers';
import GlobalStyle from '../../utils/Styles';

class NewOpsporterContainer extends React.Component {
  constructor(props) {
    super(props);
    this.newOpsporter = {};
    this.state = {
  ...
  submit = () => {
    const fieldResult = formsValidation(this.newOpsporter, 'AddOpsporter');
    if (fieldResult.warning === false) {
      this.props.actions.addOpsporter(this.newOpsporter);
    } else {
      this.setState({
    ...
    return (
      <div id="vehicleContainer">
        <Paper style={GlobalStyle.containerPaperStyle} zDepth={0}>
          <OpsporterForm setValue={this.setValue} />
        </Paper>
...
export default connect(mapStateToProps, mapDispatchToProps)(NewOpsporterContainer);
