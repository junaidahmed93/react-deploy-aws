...
export default class UpcomingDropoff extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
    ...
    if (!(_.isEmpty(bookings))) {
      rows = bookings.map(data => (
        <TableRow style={GlobalStyle.tableRowSpacing} key={data.id} className="table-row-style">
          ...
          <TableRowColumn data-uid={data.id} style={GlobalStyle.tableRowCell}>{Converters.dateConverter(data.dropOffTime)}-{Converters.timeConverter(data.dropOffTime)}</TableRowColumn>
        </TableRow>
