...
export default class DriporterTable extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleCellClick = (rowNumber, columnNumber, evt) => {
    if (evt && evt.target && evt.target.dataset && evt.target.dataset.uid) {
      ...
      browserHistory.push(`/home/admin/driporter-job/${id}`);
    }
  }
  render() {
    const rows = this.props.rows.map(data => (
      <TableRow style={GlobalStyle.tableRowSpacing} key={data.id} className="table-row-style">
        ...
        <TableRowColumn data-uid={data.id} style={GlobalStyle.tableRowCell}>{data.assignedBatchNumber || 0}</TableRowColumn>
        <TableRowColumn data-uid={data.id} style={GlobalStyle.tableRowCell}>{data.totalNumberOfCompletedBatches}</TableRowColumn>
